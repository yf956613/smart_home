...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagFangDaoBaoJin_SW)
    {
        info->event[info->num] = EVENT_FangDaoBaoJin_SW;
        info->num++;
        dataPoints->valueFangDaoBaoJin_SW = gizStandardDecompressionValue(FangDaoBaoJin_SW_BYTEOFFSET,FangDaoBaoJin_SW_BITOFFSET,FangDaoBaoJin_SW_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLED_ON_OFF)
    {
        info->event[info->num] = EVENT_LED_ON_OFF;
        info->num++;
        dataPoints->valueLED_ON_OFF = gizStandardDecompressionValue(LED_ON_OFF_BYTEOFFSET,LED_ON_OFF_BITOFFSET,LED_ON_OFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagSocket_ON_OFF)
    {
        info->event[info->num] = EVENT_Socket_ON_OFF;
        info->num++;
        dataPoints->valueSocket_ON_OFF = gizStandardDecompressionValue(Socket_ON_OFF_BYTEOFFSET,Socket_ON_OFF_BITOFFSET,Socket_ON_OFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagDoor_SW)
    {
        info->event[info->num] = EVENT_Door_SW;
        info->num++;
        dataPoints->valueDoor_SW = gizStandardDecompressionValue(Door_SW_BYTEOFFSET,Door_SW_BITOFFSET,Door_SW_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagWin_SW)
    {
        info->event[info->num] = EVENT_Win_SW;
        info->num++;
        dataPoints->valueWin_SW = gizStandardDecompressionValue(Win_SW_BYTEOFFSET,Win_SW_BITOFFSET,Win_SW_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueFangDaoBaoJin_SW != cur->valueFangDaoBaoJin_SW)
    {
        GIZWITS_LOG("valueFangDaoBaoJin_SW Changed\n");
        ret = 1;
    }
    if(last->valueLED_ON_OFF != cur->valueLED_ON_OFF)
    {
        GIZWITS_LOG("valueLED_ON_OFF Changed\n");
        ret = 1;
    }
    if(last->valueSocket_ON_OFF != cur->valueSocket_ON_OFF)
    {
        GIZWITS_LOG("valueSocket_ON_OFF Changed\n");
        ret = 1;
    }
    ...
    if(last->valueDoor_SW != cur->valueDoor_SW)
    {
        GIZWITS_LOG("valueDoor_SW Changed\n");
        ret = 1;
    }
    if(last->valueWin_SW != cur->valueWin_SW)
    {
        GIZWITS_LOG("valueWin_SW Changed\n");
        ret = 1;
    }
    if(last->valueKeRanQiYanWu_Alarm != cur->valueKeRanQiYanWu_Alarm)
    {
        GIZWITS_LOG("valueKeRanQiYanWu_Alarm Changed\n");
        ret = 1;
    }
    if(last->valueFangDaoBaojin != cur->valueFangDaoBaojin)
    {
        GIZWITS_LOG("valueFangDaoBaojin Changed\n");
        ret = 1;
    }
    ...
    if(last->valueAirQuality != cur->valueAirQuality)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueAirQuality Changed\n");
            ret = 1;
        }
    }
    if(last->valueKeRanQiYanWu != cur->valueKeRanQiYanWu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueKeRanQiYanWu Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(FangDaoBaoJin_SW_BYTEOFFSET,FangDaoBaoJin_SW_BITOFFSET,FangDaoBaoJin_SW_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFangDaoBaoJin_SW);
    gizStandardCompressValue(LED_ON_OFF_BYTEOFFSET,LED_ON_OFF_BITOFFSET,LED_ON_OFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED_ON_OFF);
    gizStandardCompressValue(Socket_ON_OFF_BYTEOFFSET,Socket_ON_OFF_BITOFFSET,Socket_ON_OFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSocket_ON_OFF);
    gizStandardCompressValue(Curtain_BYTEOFFSET,Curtain_BITOFFSET,Curtain_LEN,(uint8_t *)devStatusPtr,dataPoints->valueCurtain);
    gizStandardCompressValue(Door_SW_BYTEOFFSET,Door_SW_BITOFFSET,Door_SW_LEN,(uint8_t *)devStatusPtr,dataPoints->valueDoor_SW);
    gizStandardCompressValue(Win_SW_BYTEOFFSET,Win_SW_BITOFFSET,Win_SW_LEN,(uint8_t *)devStatusPtr,dataPoints->valueWin_SW);
    gizStandardCompressValue(KeRanQiYanWu_Alarm_BYTEOFFSET,KeRanQiYanWu_Alarm_BITOFFSET,KeRanQiYanWu_Alarm_LEN,(uint8_t *)devStatusPtr,dataPoints->valueKeRanQiYanWu_Alarm);
    gizStandardCompressValue(FangDaoBaojin_BYTEOFFSET,FangDaoBaojin_BITOFFSET,FangDaoBaojin_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFangDaoBaojin);
    ...
    devStatusPtr->valueAirQuality = exchangeBytes(gizY2X(AirQuality_RATIO,  AirQuality_ADDITION, dataPoints->valueAirQuality)); 
    devStatusPtr->valueKeRanQiYanWu = exchangeBytes(gizY2X(KeRanQiYanWu_RATIO,  KeRanQiYanWu_ADDITION, dataPoints->valueKeRanQiYanWu)); 
