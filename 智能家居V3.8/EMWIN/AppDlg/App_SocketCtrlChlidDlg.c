...
OS_TMR 	g_Sockettmr2;		//定时器2
void g_Sockettmr2_callback(void *p_tmr, void *p_arg); 	//插座定时器回调函数

static unsigned char g_ucLampNumber = 0;  //选中灯的编号
static unsigned int g_u32TimerToSet;   //存放SPINBox设置的时间
static unsigned int g_CountSec = 0;  //用来计数周期定时器经过了多少秒
static char g_ucBuf[6];
unsigned char g_ucSocketStatus = 0;  //插座的状态
unsigned char g_bSocketTimerFlag;
extern unsigned char SocketOnOfflag;

WM_HWIN g_CountSechItem;
  ...
  case WM_NOTIFY_PARENT:
    ...
    case ID_DROPDOWN_0: // Notifications sent by 'SocketDropdown'
      ...
      case WM_NOTIFICATION_SEL_CHANGED:
				g_ucLampNumber = DROPDOWN_GetSel(WM_GetDialogItem(pMsg->hWin, ID_DROPDOWN_0));
				DROPDOWN_GetItemText(WM_GetDialogItem(pMsg->hWin, ID_DROPDOWN_0), g_ucLampNumber,g_ucBuf,sizeof(g_ucBuf));    
        break;
      }
      break;
      ...
      switch(NCode) 
			{
				case WM_NOTIFICATION_RELEASED:
					...
					if(g_u32TimerToSet > 0 && g_ucSocketStatus > 0)  //定时时间和选中开或关才开启定时器
					{	
						if(OSTmrStateGet ((OS_TMR  *)&g_Sockettmr2,(OS_ERR  *)&err) == OS_TMR_STATE_UNUSED)  //如果定时器没有创建
						{
							g_bSocketTimerFlag = 1;
							OSTmrCreate((OS_TMR		*)&g_Sockettmr2,		//定时器1
														(CPU_CHAR	*)"g_Sockettmr2",		//定时器名字
														(OS_TICK	 )0,			//20*10=200ms
														(OS_TICK	 )100,          //100*10=1000ms
														(OS_OPT		 )OS_OPT_TMR_PERIODIC, //周期模式
														(OS_TMR_CALLBACK_PTR)g_Sockettmr2_callback, //定时器1回调函数
														(void	    *)0,			//参数为0
														(OS_ERR	    *)&err);		//返回的错误码		
							OSTmrStart(&g_Sockettmr2,&err);	//开启定时器1
							GUI_EndDialog(pMsg->hWin, 0);  //关闭对话框
					 }
					}
    ...
    case ID_BUTTON_2: // Notifications sent by 'CancleButton'
      switch(NCode) 
			{
      case WM_NOTIFICATION_RELEASED:
				if(OSTmrStateGet ((OS_TMR  *)&g_Sockettmr2,(OS_ERR  *)&err) != OS_TMR_STATE_UNUSED) //定时器已经创建
				 {
						OSTmrDel ((OS_TMR  *)&g_Sockettmr2,(OS_ERR  *)&err);  //取消定时-删除定时器
				 }
        break;
      }
      break;
...
void App_SocketCtrlChlidDlg(WM_HWIN hWin) 
{
  GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, hWin, 0, 0);
}

extern unsigned int SocketCountdownSec;
extern unsigned char SocketOnOfflag;  //插座开关的标志位
//定时器的回调函数
void g_Sockettmr2_callback(void *p_tmr, void *p_arg)
{
	OS_ERR  err;
	g_CountSec++;
//	char RX_TX_Buf[10];
	SocketCountdownSec = g_u32TimerToSet*60 - g_CountSec;
	if(g_CountSec >= (g_u32TimerToSet*60))
	{
		g_CountSec = 0;
		g_bSocketTimerFlag = 0;
		g_u32TimerToSet = 0;

		if(g_ucSocketStatus == 1)   //定时开
		{
			SOCKET_SW = 1;
			SocketOnOfflag = 1;
		}
		if(g_ucSocketStatus == 2)   //定时关
		{
			SOCKET_SW = 0;
			SocketOnOfflag = 0;
		}
		OSTmrDel ((OS_TMR  *)&g_Sockettmr2,(OS_ERR  *)&err);  //定时时间到,删除定时器
	}
}
